/**
 * Sprint 2 seed: Beer style groups, beer styles, mashing profiles
 * Run with: node scripts/seed-sprint2.mjs
 */
...
// ── Beer Style Groups ───────────────────────────────────────
const groups = [
  { name: "Czech Lager", sort_order: 1 },
  { name: "International Lager", sort_order: 2 },
  { name: "Pale Ale / IPA", sort_order: 3 },
  { name: "Wheat Beer", sort_order: 4 },
  { name: "Stout / Porter", sort_order: 5 },
  { name: "Belgian", sort_order: 6 },
  { name: "Sour / Wild", sort_order: 7 },
  { name: "Specialty", sort_order: 8 },
];

console.log("Seeding beer style groups...");
for (const g of groups) {
  await sql`
    INSERT INTO beer_style_groups (name, sort_order)
    VALUES (${g.name}, ${g.sort_order})
    ON CONFLICT DO NOTHING
  `;
}

// Get group IDs
const groupRows = await sql`SELECT id, name FROM beer_style_groups`;
const groupMap = new Map(groupRows.map((r) => [r.name, r.id]));
console.log(`  ✓ ${groupMap.size} groups`);

// ── Beer Styles ─────────────────────────────────────────────
// [bjcp, category, name, abvMin, abvMax, ibuMin, ibuMax, ebcMin, ebcMax, ogMin, ogMax, fgMin, fgMax, groupName]
const styles = [
  // Czech Lager
...
console.log("Seeding beer styles...");
let styleCount = 0;
for (const [bjcp, cat, name, abvMin, abvMax, ibuMin, ibuMax, ebcMin, ebcMax, ogMin, ogMax, fgMin, fgMax, groupName] of styles) {
  const groupId = groupMap.get(groupName);
  if (!groupId) { console.warn(`  ! No group: ${groupName}`); continue; }
  await sql`
    INSERT INTO beer_styles (style_group_id, bjcp_number, bjcp_category, name, abv_min, abv_max, ibu_min, ibu_max, ebc_min, ebc_max, og_min, og_max, fg_min, fg_max)
    VALUES (${groupId}, ${bjcp}, ${cat}, ${name}, ${abvMin}, ${abvMax}, ${ibuMin}, ${ibuMax}, ${ebcMin}, ${ebcMax}, ${ogMin}, ${ogMax}, ${fgMin}, ${fgMax})
    ON CONFLICT DO NOTHING
  `;
  styleCount++;
}
console.log(`  ✓ ${styleCount} styles`);
...
console.log("Seeding mashing profiles...");
for (const p of profiles) {
  await sql`
    INSERT INTO mashing_profiles (tenant_id, name, steps, notes)
    VALUES (NULL, ${p.name}, ${JSON.stringify(p.steps)}::jsonb, ${p.notes})
    ON CONFLICT DO NOTHING
  `;
}
