...
### Aktualizováno: 06.03.2026 | Poslední sprint: Sprint 9 (SaaS + Deployment)
...
**Flow:**
1. Uživatel zadá: jméno, účel (profesionální / domácí pivovarník), název pivovaru, email, heslo
2. **Professional:** Systém vytvoří: tenant → user_profile → tenant_user (role=owner) → subscription (plan=Pro, status=trial, trial 30 dní) + default provozovna + default sklad + tenant settings (CZK, cs, Europe/Prague, excise defaults) + 6měsíční overage waiver (launch promo)
3. **Homebrewer:** subscription (plan=community_homebrewer, status=active) — bez trialu, trvale zdarma do 2 hl/rok
...
### 8.5 Billing ✅
- Aktuální plán a subscription status (active/trial/expired)
