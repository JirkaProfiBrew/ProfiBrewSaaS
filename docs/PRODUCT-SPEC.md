...
### Aktualizováno: 07.03.2026 | Poslední sprint: Sprint 9 Patch 9.1/9.2 (Onboarding, Trial Conversion, Pilots)
...
**Flow (aktualizováno Sprint 9 Patch):**
1. Uživatel vybere plán na `/pricing` (4 komerční + 2 community plány)
2. Redirect na `/register?plan=<slug>` — formulář zobrazí vybraný plán nahoře
3. Uživatel zadá: jméno, název pivovaru, email, heslo (purpose se odvozuje z planSlug)
4. Systém vytvoří: tenant → user_profile → tenant_user (role=owner) → subscription
5. **Provisioning (rozšířený seed):**
   - 3 sklady: Suroviny (SUR, raw_materials), Pivo (PIVO, beer, excise relevant), Ostatní (OST, other)
   - Shop settings: stockMode (dle plánu), warehouse IDs, ekonomické parametry (režie 15%, fixní 1000 Kč, batch 2000 Kč)
   - Pokladna "Hlavní pokladna"
   - Cashflow kategorie z globálních templates (2 kořeny + 9 podkategorií)
   - 6 číslovacích řad + 3 zálohy
6. **Plánové varianty:**
   - Free: status='active', bez trialu, stockMode='none'
   - Starter/Pro/Business: status='trialing', trial 30 dní, overage waiver 6 měsíců
   - Community homebrewer: status='active', trvale zdarma, stockMode='liters'
7. Redirect na `/onboarding` (6-krokový wizard)

**Pilot registrace:**
- URL: `/register?invite=<token>` — email předvyplněn (readonly), plán z pozvánky
- source='pilot', inviteId uložen na subscription

**Pravidla:**
- Email musí být unikátní
- Heslo: min 6 znaků
- Název pivovaru → automaticky generovaný slug (URL-safe)
- Registrace bez ?plan= parametru → redirect na /pricing
- Registrace škol: odkaz na kontaktní formulář (`/contact/school`)

### 1.1a Pricing Page ✅

**Route:** `/pricing` (veřejná, bez auth)

4 komerční plány v gridu (Free 0 / Starter 1 490 / Pro 3 490 / Business 6 990 CZK/měsíc) + oddělená community sekce (Domovarník, Škola). Plány načteny z DB. CTA tlačítka vedou na `/register?plan=<slug>`.

### 1.1b Onboarding Wizard ✅

**Route:** `/onboarding` (auth required, mimo dashboard layout)

6 kroků:
1. **Vítejte** — shrnutí auto-vytvořených dat, [Začít] / [Přeskočit]
2. **Váš pivovar** — název, adresa, IČO, DIČ (v tenant.settings)
3. **Sklady** — 3 předvytvořené sklady, stock mode volba (litry/obaly)
4. **Ekonomické parametry** — režie %, fixní režie, batch cost, generovat výdaj z příjemky
5. **Spotřební daň** — toggle + info o sazbách
6. **Hotovo** — souhrn, [Přejít na dashboard]

**Přeskočení:** nastaví onboarding_skipped=true, dashboard zobrazí reminder banner
**Reminder banner:** "Průvodce nastavením jste přeskočili." + [Spustit] / [Příště nezobrazovat]
...
### 8.5 Billing ✅
- Aktuální plán a subscription status (active/trialing/pending_payment/past_due)
- Trial progress bar (zbývající dny, žlutá/červená barva)
- Seznam dostupných modulů (✅/❌)
- Limity plánu (objem v hl, uživatelé)
- CTA na upgrade stránku + kontaktní email
- **Změna plánu v trialu** ✅ — select z dostupných plánů, preview modulů, trial_ends_at se nemění
- **Pending payment stav** ✅ — žlutý badge + platební instrukce (částka, účet, VS)
- **Past due stav** ✅ — červený badge + kontaktní info
- 💡 Stripe integrace (Sprint 12)

### 8.5a Post-trial Conversion ✅
- **Conversion modal** — zobrazí se při prvním přihlášení po expiraci trialu
- Výběr plánu (pokračovat/přejít na jiný), consent checkbox
- Po potvrzení: subscription status → pending_payment, billing_event záznam
- Grace period: 14 dní od trial_ends_at (admin řeší ručně)
- Pending payment: plný přístup + žlutý banner v dashboardu
...
- URL: /upgrade?module=stock (parametr pro zvýraznění blokovaného modulu)
- Zobrazí aktuální plán (badge "Aktuální plán")
- Porovnávací tabulka všech veřejných plánů (Free/Starter/Pro/Business)
- Pro plán zvýrazněn jako "Doporučujeme"
- Moduly: ✅/❌ per plán, limity (hl, uživatelé), extras (API, integrace, podpora)
- CTA: kontaktní sekce s emailem (Stripe přijde v S12)

### 9.4 Trial banner ✅

- Žlutý banner v dashboard layoutu: zobrazí se ≤7 dní do konce trialu
- Červený banner: trial vypršel ("Zkušební období skončilo: {datum}")
- Žlutý banner pro pending_payment: "Čeká na platbu — fakturu jsme odeslali na váš email"
- Odkaz na /settings/billing
...
- Sidebar sekce: SaaS Monitor (dashboard), Systémové browsery (rmutovací profily, CF Templates), Pilot & Billing (Piloti, Billing)
...
### 10.6 Cashflow Templates ✅
- **Route:** `/admin/cashflow-templates`
- Tree view globálních šablon kategorií (income/expense, parent-child)
- CRUD: přidání kořenové/podkategorie, editace, soft delete (isActive=false)
- Ovlivňuje pouze nové registrace (ne existující tenanty)
- Seed: 2 kořeny (Příjmy, Výdaje) + 9 podkategorií

### 10.7 Pilot Invitations ✅
- **Route:** `/admin/pilots`
- Tabulka pozvánek: email, plán, dní trialu, speciální cena, status, akce
- Vytvoření pozvánky: email, plán (default Pro), trial days, price override, poznámky
- Kopírování invite URL: `https://app.profibrew.com/cs/register?invite=<token>`
- Statusy: pending → sent → registered / expired
- Token: 32-char random hex, expirace 90 dní

### 10.8 Billing Events ✅
- **Route:** `/admin/billing`
- Tabulka billing eventů: pivovar, typ (conversion_confirmed, etc.), plán, částka, datum, status
- Akce: "Zpracovat" → processed=true + processedAt + processedBy
- MVP: admin ručně zpracovává platby, Stripe přijde v S12
...
| — | Module Access | Upgrade page + ModuleGuard + Trial banner | ✅ |
...
| cashflow_category_templates | Admin | CF Templates | ✅ |
| pilot_invitations | Admin | Piloti | ✅ |
