-- Standalone script to seed cashflow_category_templates on PROD
-- Run this in Supabase SQL Editor if migration 0031 hasn't been applied yet.
-- Safe to run multiple times (ON CONFLICT DO NOTHING + idempotent soft-delete).

-- 1. Soft-delete any old templates
UPDATE cashflow_category_templates SET is_active = false WHERE is_active = true;

-- 2. Insert roots first (no parent_id), then children
...
-- Verify
SELECT id, name, cashflow_type, parent_id, sort_order, is_active
FROM cashflow_category_templates
WHERE is_active = true
ORDER BY cashflow_type, sort_order, parent_id NULLS FIRST;
